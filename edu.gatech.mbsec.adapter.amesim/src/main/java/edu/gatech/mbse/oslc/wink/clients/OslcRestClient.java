...
public final class OslcRestClient
{
	...
	public ClientResponse getOslcResource(String eTag)
    {
	    try
	    {
	    	final ClientResponse response;
	    	if(eTag == null){
	    		response = clientResource.accept(mediaType).get();
	    	}
	    	else{
	    		response = clientResource.header("If-None-Match", eTag).accept(mediaType).get();
	    	}
	...
	public ClientResponse headOslcResource(String eTag)
    {
	    try
	    {
	    	final ClientResponse response;
    	    if(eTag != null){
    	    	response = clientResource.header("If-None-Match", eTag).accept(mediaType).head();
    	    }
    	    else{
    	    	response = clientResource.accept(mediaType).head();
    	    }	    	
    	    return response;
	    }
	    catch (final ClientWebException exception)
    ...
    public ClientResponse updateOslcResourceReturnClientResponse(final Object oslcResource, String eTag)
    {
        try
        {
            return clientResource.header("If-Match", eTag).contentType(mediaType).put(ClientResponse.class, oslcResource);
        }
        catch (final ClientWebException exception)
        {
            final ClientResponse response = exception.getResponse();
